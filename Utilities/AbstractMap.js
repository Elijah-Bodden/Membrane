	...
	async findNextHop(currentNode, endNode) {
		...
		while (node != currentNode) {
			...
			if (this.distances[node] == Infinity)
				throw new Error(`The requested route <${currentNode} -> ${endNode}> is not possible in the current map (there exists no path between intermediary node ${node} and the requested ${currentNode}).`);
