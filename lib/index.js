...
async function defaultConfigLoadFunction() {
...
class peerConnection {
	...
	async handleMessage(message) {
		...
		switch (parsed[0]) {
			case "consumable":
				if (!this.peerData?.hiddenAlias || !this.isAuth) return;
...
async function init(configLoadFunction) {
	await loadConfig(defaultConfig, configLoadFunction ?? defaultConfigLoadFunction);
...
async function sendConsumable(hiddenAlias, raw_data) {
	if (!authPeers.includes(hiddenAlias)) {
		throw new Error("No authenticated connection to peer " + hiddenAlias)
	}
	livePeers[hiddenAlias].transport.channel.standardSend("consumable", {raw: raw_data})
}
