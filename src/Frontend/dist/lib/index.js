...
var defaultConfig = {
	...
	constants : {
		...
		configLoadFunction : async function() {
			...
			console.log(JSON.parse(window.localStorage.config ?? "{}"))
...
class peerConnection {
	...
	async makeDefiniteRoute(destination, desiredPermissions) {
		...
		console.log("RESULT^^^^", result.signalIdentifier.split("|")[0])
		switch (result.signalIdentifier.split("|")[0]) {
			...
			case "routeAccepted":
				...
				indicateRouteAccepted(destination) //&89
		}
	}
	...
	messageMethods = {
		invokerIntroduction: async (message) => {
			try {
				...
				LOG(`Received invalid initalization message for hiddenAlias ${message.hiddenAlias}, delineated by the reason ${error}; terminating connection.`);
				peerConnection.prototype.close(this);
				return;
			}
...
async function publicError(message, stack) {
	alert(`Something went wrong!\nWe appologize greatly for this inconvenience. If you wish to proceed as normal, please simply reload the page; if the issue persists, feel free to send a message to membranexus@gmail.com containing the following error text: ${message} \n[stack]:\n${stack}`)
}
