...
window.addEventListener("DOMContentLoaded", async function () {
  ...
  document.querySelector("#chatEntryField").style.height =
    document.querySelector("#chatEntryField").scrollHeight;
  document.querySelector("#chatEntryField").scrollTop =
    document.querySelector("#chatEntryField").style.scrollHeight;
});
...
window.onload = function () {
  ...
  document
    .querySelector("#searchEntryField")
  ...
  document
    .querySelector("#searchEntryField")
    .addEventListener("focusin", async (e) => {
  ...
  document
    ...
    .addEventListener("focusout", async () => {
      setTimeout(() => {
        if (
          !(
            document.querySelector("#searchEntryField") ==
  ...
  document
    .querySelector("#connectedNodesPane")
  ...
  Array.from(document.querySelectorAll(".integerEntryContainer")).forEach(
    (container) => {
  ...
  Array.from(document.querySelectorAll(".settingsToggle")).forEach((toggle) => {
    toggle.addEventListener("change", () => {
      if (toggle.dataset.objective === "doTURN") {
        ...
        writeSetting(
          "rtc.ICEpresets.iceServers",
          toggle.checked ? STUNOnly : allICEServers
        );
        return;
      } else if (
        toggle.dataset.objective ===
        "communication.moderateMapInstabilityTolerance"
      ) {
  ...
  onPublicError(async (_sig, externalDetail) => {
    ...
    document
      .querySelectorAll("body > *:not(.fatalWrapper):not(.dontBlur)")
      .forEach((element) => {
    ...
    document.querySelector(
      ".fatalBody"
    ).innerHTML = `<div class="center">Oh no! Something went wrong!</div><br><br>That's all we know. We apologize sincerely for this terrible inconvenience. If you wish to try and proceed with the service as normal, simply reload the page. If this error persits, feel free to email <b>Membranexus@gmail.com</b> the following error text:<div class="center"></div><br>${externalDetail[0]}<br>[stack]<br>${externalDetail[1]}</div>`;
  ...
  if (
    !JSON.parse(localStorage.config ?? "{}")["UI.renderUnfamiliarPublicAliases"]
  ) {
  ...
  connectioncardcontext = new standardContextMenu(
    "connection-card-context",
    "connectedNodesPane"
  );
  ...
  onAuthPeersUpdated((_signalID, externalDetail) => {
    if (externalDetail[0] == "addition") {
      if (livePeers[externalDetail[1]]) {
        ...
        if (eventStream.dontAlert.includes(externalDetail[1])) {
          eventStream.dontAlert.splice(
            eventStream.dontAlert.indexOf(externalDetail[1]),
            1
          );
        } else {
          eventStream.log(
        ...
        if (
          !eventHandler.handlerFrame[
            `consumableAuth | ${livePeers[externalDetail[1]].internalUID}`
          ]
        )
          livePeers[externalDetail[1]].onConsumableAuth(
            (_signal, _externalDetail_) => {
    ...
    if (!(fusedStream.chatCache?.[externalDetail[1]]?.isActive ?? true)) {
      ...
      document
        .querySelector(".sendbutton")
        .style.setProperty("filter", "brightness(330%)");
    } else {
      document
        .querySelector(".sendbutton")
...
class RightPanel {
  ...
  rotateIn(target) {
    ...
    if (target === "messageFeedPane" && target != initialState)
      fusedStream.refresh();
  }
}
...
class FusedStream {
  constructor(paneID, messageSequenceSelector, isInteractive) {
    ...
    this.pane = paneID;
    this.messageSequenceContainer = document.querySelector(
      messageSequenceSelector
    );
    this.chatCache = {};
  ...
  async chatInit() {
    if (!JSON.parse(localStorage.config ?? "{}")["communication.hiddenAlias"]) {
      ...
      eventHandler.onReceipt(
        "configLoaded",
        this.loadDefaultWelcomeSequence.bind(this)
      );
    }
  }
  async streamInit() {
  ...
  async loadCache(ID) {
    if (this.chatCache[ID] && this.panel.activePane === this.pane) {
      this.messageSequenceContainer.innerHTML = this.chatCache[
        ID
      ].exchange.reduce((previous, message, index) => {
        return (
          this.formatMessageCard(message, index, this.chatCache[ID]) + previous
        );
      }, "");
    ...
    document.querySelector(
      ".chattingwithheader"
    ).innerHTML = `Chatting with ${this.getRenderableID(ID)}:`;
    this.activeChat = ID;
  }
  getRenderableID(ID) {
  ...
  async log(ID, data, polarity, tags = [], adhoc = "") {
    ...
    data =
      ID === "system" && polarity === "transient" ? "[system]: " + data : data;
    var index =
    ...
    if (this.panel.activePane === this.pane && this.activeChat === ID) {
      this.messageSequenceContainer.innerHTML =
        this.formatMessageCard(
          this.chatCache[ID].exchange[index],
          index,
          this.chatCache[ID]
        ) + this.messageSequenceContainer.innerHTML;
    }
  ...
  formatMessageCard(renderPackage, index, cache, adhoc) {
    ...
    if (renderPackage.tags.includes("system-message-card-route-pending")) {
      return this.formatConfirmationCard(
        renderPackage,
        index,
        cache,
        renderPackage.adhoc
    ...
    const card = `<div class="${renderPackage.tags.join(
      " "
    )} message-card message-card-${renderPackage.polarity}">
    ...
    if (renderPackage.tags.includes("message-card-unread")) {
      renderPackage.tags.splice(
        renderPackage.tags.indexOf("message-card-unread"),
        1
      );
    }
    return card;
  ...
  async downloadContent(ID) {
    const raw = this.chatCache[ID].exchange.reduce((sum, component) => {
      return (
        sum +
        `${this.getRenderableID(
          this.chatCache[ID].members[component.polarity]
        )} (${component.polarity}): ${component.data}\n`
      );
    }, "");
...
class standardContextMenu {
  ...
  async raceDismissEvents() {
    ...
    const promiseConsequence = (_) => {
      ...
      this.menu.removeEventListener(
        "mouseup",
        triggerFunctions.clickContextEvent
      );
    ...
    Promise.race(
      ["windowResize", "contextButtonClicked", "nonContextClick"].map(
        (listener) =>
  ...
  click(event) {
    ["data-reference", "data-index", "data-content", "data-sender"].forEach(
      (propery) =>
...
class connectionsPanel {
  ...
  renderConnectionCards() {
    ...
    authPeers.forEach((connection) => {
      ...
      this.container.innerHTML =
        this.formatConnectionCard(connection) + this.container.innerHTML;
    });
  }
}
    ...
    .addEventListener("paste", (event) => {
      ...
      if (
        event.target.value.length +
          (event.clipboardData || window.clipboardData).getData("text").length >
        32
      ) {
        flashLock = +new Date();
        event.target.parentElement.style.setProperty(
...
function getRenderableDate(UTCString) {
  ...
  if (
    JSON.stringify(localTime.slice(1, 3)) ==
    JSON.stringify(new Date().toString().split(" ").slice(1, 3))
  ) {
    if (
      parseInt(localTime[4].split(":")[0]) >= 12 &&
      parseInt(localTime[4].split(":")[0]) != 24
    ) {
      return `${
    ...
    const amHours =
      parseInt(localTime[4].split(":")[0]) == 0
        ? 12
        : parseInt(localTime[4].split(":")[0]);
...
async function toggleDescription(event) {
  ...
  var explanation = Array.from(event.target.parentNode.childNodes).filter(
    (element) => {
      if (!element.matches) return false;
...
async function fillDefaults(shortCircuit) {
  ...
  Array.from(document.querySelectorAll(".configCardBool .settingsToggle"))
    ...
    .forEach((element, index) => {
      ...
      writeSetting(
        element.dataset.objective,
        index === 1 ? !element.checked : element.checked
      );
    });
