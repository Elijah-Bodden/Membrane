			...
			[1, 0]
				.map((flippedIndex, trueIndex) => {
					return (
						...
						graph.extremities(edge)[trueIndex] === CONFIG.communication.hiddenAlias
					);
				})
				.includes(true)
		) {
		...
		for (let len in sortedSuggestions) {
			...
			for (let suggestion of sortedSuggestions[len]) {
				...
				const boldedSuggestion =
					...
					`<b>${hiddenAliasLookup[suggestion].slice(firstIndex, firstIndex + lcQuery.length)}</b>` +
...
async function renderQuerySuggestions(suggestionPairs, suggestionPriority, isInformed) {
	var suggestionPairs =
		typeof suggestionPairs === "object" ? suggestionPairs : Object.assign(hiddenAliasLookup);
    ...
    <li class="context-item">
      <button class="context-button border-surround ${
		...
		}suggestion-button" id="${hidden}-suggestion-button" onclick="(() => {activateNode('${hidden}'); if (res.lastCohort==='small') rightPanel.rotateIn('visualizationPane')})()" data-index=${index}>
	...
	document.querySelectorAll(".suggestion-button").forEach((btn) => {
		...
		btn.addEventListener("keydown", async (e) => {
			if (e.key == "ArrowDown") {
				e.preventDefault();
				if (
					document.querySelectorAll(".suggestion-button")[new Number(e.target.dataset.index) + 1] ==
					undefined
				)
					return;
				document
	...
	document.querySelector(".search-opts-cap").addEventListener("keydown", async (e) => {
		if (e.key == "ArrowUp") {
			e.preventDefault();
...
async function raceNodeContextDismissEvents(context) {
	...
	let resizeCall = nodeDismissHandler.acquireExpectedDispatch("windowResize", 60000);
	let contextClickCall = nodeDismissHandler.acquireExpectedDispatch("contextButtonClicked", 60000);
	let nonContextClickCall = nodeDismissHandler.acquireExpectedDispatch("nonContextClick", 60000);
	let wheelStageCall = nodeDismissHandler.acquireExpectedDispatch("wheelStage", 60000);
	let raceTimeoutDefault = nodeDismissHandler.acquireExpectedDispatch("defaultRaceResolveTimer", 59000);
...
async function nodeClick(event) {
	...
	if (recentlyClickedNode === event.node) {
		rightClickNodeAction(event, true);
		console.log("test", event);
		recentlyClickedNode = null;
		return;
	}
	recentlyClickedNode = event.node;
	var currentClickTimestamp = +new Date();
	lastClickTimestamp = currentClickTimestamp;
	setTimeout(() => {
		if (currentClickTimestamp === lastClickTimestamp && recentlyClickedNode) {
			currentClickTimestamp = null;
			recentlyClickedNode = null;
		}
	}, 5000);
}

renderer.on("clickNode", nodeClick);

renderer.on("clickStage", async () => {
	activateNode();
});

async function rightClickNodeAction(event, doubleTapReferred) {
	activateNode(event.node);
	if (event.node === "server") {
		return;
	}
	if (doubleTapReferred) {
		setTimeout(() => {
	...
	const contextPromise = nodeDismissHandler.acquireExpectedDispatch("mouseUpMenuSpawnable");
	generateNodeContext(event.node, authPeers.includes(event.node), contextPromise);
}

renderer.on("rightClickNode", rightClickNodeAction);

renderer.on("doubleClickNode", async (event) => {
