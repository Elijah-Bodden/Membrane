...
const clients = {};
const clientAges = {};
const defaultNoResponseTimeout = 100000;
var logger;
...
webSocket.prototype.crudeSend = async function (type, typeArgs) {
  ...
  switch (type) {
    ...
    case "requestSDP":
      ...
      this.send(
        JSON.stringify(["requestSDP", typeArgs.SDP, typeArgs.returnID])
      );
      break;
    case "ERROR":
  ...
  setInterval(() => {
    for (let client of Object.values(clients)) {
      if (client.socket.readyState === webSocket.OPEN)
        client.socket.crudeSend("heartbeat");
    }
...
websocketServer.on("upgrade", async (req, socket, head) => {
  ...
  logger.diagnostic(
    `New websocket connection on ${routeURL.pathname} from ${req.socket.remoteAddress}.`
  );
  if (
    (routeURL.pathname !== "/bind" && routeURL.pathname !== "/reconnect") ||
    (routeURL.pathname === "/bind" &&
      !routeURL.searchParams.get("originatingSDP"))
  ) {
    socket.destroy();
    return;
  }
      ...
      switch (parsed[0]) {
        ...
        case "returnSDP":
          ...
          eventHandler.dispatch(
            `serialRequestResponse|${this.CID}|${parsed[2]}`,
            parsed[1]
          );
          break;
        case "ignoreSDPRequest":
          ...
          eventHandler.forceReject(
            `serialRequestResponse|${this.CID}|${parsed[1]}`
          );
...
async function fetchArbitraryLinkSDP(initSDP, recursionDepth) {
  ...
  if (nominee?.socket?.readyState !== webSocket.OPEN)
    await terminateAndCleanClient(nomineeIndex);
  if (!nominee) {
