...
## What  is this?
Membrane creates hardy, living WebRTC-based peer networks. It takes signalling to the browser. After just one initial server-based signal, a node never again needs centrality. The network acts as one giant singaling *membrane*, connecting far-flung peers at a whim. So long as a node remains a part of the network, it has full contact with every participant. Meanwhile, the network actively stabilizes around each new member, ensuring that severed connections will not damage the network nor cut off nodes, and allowing for an overall seemless "immediate-access-to-anything" experience.
